...
export function openDb(path = DB_PATH, { config } = {}) {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS workstreams (
      ...
      pr_checked_at TEXT,
      label TEXT,               -- optional display name override (set via fw rename)
...
export function upsertWorkstream(db, fw) {
  const status = fw.status === 'paused' ? 'paused' : 'active';
  const uuid = String(fw.uuid || newWorkstreamUuid()).toLowerCase();
  ...
  `).run(uuid, fw.org, fw.repo, fw.branch, fw.path, fw.source, status, fw.created_at, fw.last_joined_at);
  return db.prepare('SELECT * FROM workstreams WHERE org=? AND repo=? AND branch=?')
    .get(fw.org, fw.repo, fw.branch);
}
...
// that fw owns the local side of a stack (this column + `fw join` to move
...
export function ensureBareClone(org, repo, config = CONFIG) {
  ...
  if (!existsSync(bare)) {
    ...
    // sets up tracking. Branches fw itself creates become the only local heads.
...
export async function linkPrAsync(db, row, options = {}) {
  if (isScratch(row)) return null;
  // Session names survive branch changes inside their worktrees.
  const branch = row.path ? await new Promise((resolve) => {
    const run = options.run || execFile;
    run('git', ['symbolic-ref', '--quiet', '--short', 'HEAD'], {
      cwd: row.path, encoding: 'utf8', timeout: 5_000,
    }, (error, stdout) => resolve(error ? null : stdout.trim() || null));
  }) : null;
  if (!branch) return null;
  const pr = await prForBranchAsync(row.org, row.repo, branch, options);
  return recordPrCheck(db, row, pr, options);
}
...
const isForkRouted = (bare) => gitTry(['--git-dir', bare, 'config', '--get', 'fw.useFork']) === '1';
...
export function linkedSessionSeed(kind, links) {
  ...
  return [
    `This is a new fw session to work on a ${target}. The following links are associated with this session. Use the linear skill with the cli and/or the gh cli to retrieve authed information.`,
