...
export const RESOURCE_KINDS = ['link', 'markdown', 'html'];
...
export function initializePanelSchema(db) {
  const schema = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='resource_associations'").get()?.sql;
  if (schema && !schema.includes("'html'")) {
    // Rebuild the CHECK constraint without cascading away existing panels.
    const foreignKeys = db.prepare('PRAGMA foreign_keys').get().foreign_keys;
    db.exec('PRAGMA foreign_keys = OFF');
    db.exec('BEGIN IMMEDIATE');
    try {
      db.exec(schema.replace('resource_associations', 'resource_associations_html')
        .replace("'link', 'markdown'", "'link', 'markdown', 'html'"));
      db.exec(`
        INSERT INTO resource_associations_html SELECT * FROM resource_associations;
        DROP TABLE resource_associations;
        ALTER TABLE resource_associations_html RENAME TO resource_associations;
      `);
      if (db.prepare('PRAGMA foreign_key_check').all().length) throw new Error('HTML resource migration violated foreign keys');
      db.exec('COMMIT');
    } catch (error) {
      db.exec('ROLLBACK');
      throw error;
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS resource_associations (
      ...
      kind TEXT NOT NULL CHECK(kind IN ('link', 'markdown', 'html')),
      value TEXT NOT NULL,
      label TEXT NOT NULL,
...
function validateGroupPanelKind(db, group, kind, resourceId = null) {
  ...
  if (kind === 'markdown' || kind === 'iframe') {
    ...
    const expected = kind === 'markdown' ? ['markdown'] : ['link', 'html'];
    if (!expected.includes(resource.kind)) throw new PanelModelError(400, `${kind} panels require a ${expected.join(' or ')} resource`);
  }
}
...
function normalizeFile(group, value, kind, { home = process.env.HOME || '', cwd = process.cwd() } = {}) {
  const label = kind === 'html' ? 'HTML' : 'Markdown';
  if (typeof value !== 'string' || !value.trim() || value.includes('\0')) {
    throw new PanelModelError(400, `${label} path must be a non-empty string`);
  }
  ...
  const valid = kind === 'html' ? /\.html?$/i.test(path) : path.toLowerCase().endsWith('.md');
  if (!valid) throw new PanelModelError(400, kind === 'html' ? 'resource must be an .html or .htm file' : 'resource must be a .md file');
  if (!existsSync(path)) throw new PanelModelError(404, `no such ${label} file: ${path}`);
  let stats;
  try { stats = statSync(path); } catch { throw new PanelModelError(404, `no such ${label} file: ${path}`); }
  if (!stats.isFile()) throw new PanelModelError(400, `${label} resource must be a regular file`);
  return path;
}
...
function insertResource(db, group, { kind, value, label, source = 'explicit' }, options = {}) {
  ...
  if (kind === 'link') {
    ...
    normalized = normalizeFile(group, value, kind, options);
  }
