...
export function resolveConfig({
  ...
  const pathEnv = {
    repositories: firstDefined(env.FRITZWORKS_REPOSITORIES, env.FW_REPOSITORIES),
    scratchpads: firstDefined(env.FRITZWORKS_SCRATCHPADS, env.FW_SCRATCHPADS),
    notes: firstDefined(env.FRITZWORKS_NOTES, env.FW_NOTES),
  ...
  // Keep existing databases in place until the user moves or overrides them.
  const legacyData = join(dataHome, 'ws');
  if (pathEnv.data === undefined && userFile.paths?.data === undefined
      && paths.data === join(dataHome, 'fritzworks')
