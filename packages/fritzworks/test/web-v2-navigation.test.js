...
test('HTML previews use the owning daemon and never mount an editor', async (t) => {
  ...
  const mounted = await mountReact(render({ id: 'remote', url: 'http://127.1.1.2:7337' }));
  t.after(async () => { await mounted.unmount(); teardownJsdom(dom); });
  const { container } = mounted;
  const frame = container.querySelector('iframe');
  assert.equal(frame.src, 'http://127.1.1.2:7337/resource-files/html-preview/preview%20one.html');
  ...
  await mounted.update(render({ id: 'local', url: null }));
  assert.equal(container.querySelector('iframe').getAttribute('src'), '/resource-files/html-preview/preview%20one.html');
});
