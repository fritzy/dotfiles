...
test('HTML schema migration preserves existing panels and opens HTML without an editor', (t) => {
  const { db, root } = fixture(t);
  const { group } = ensureSessionPanelGroup(db, { id: 'html-test', type: 'configured', name: 'HTML', path: root }, { bump: true });
  const link = addResource(db, group.id, { kind: 'link', value: 'https://example.com' }, readPanelLayout(db).revision);
  openResourcePanel(db, link.resource.id, {}, link.revision);
  const before = readPanelLayout(db);
  const schema = db.prepare("SELECT sql FROM sqlite_master WHERE name='resource_associations'").get().sql;
  db.exec('PRAGMA foreign_keys = OFF');
  db.exec(schema.replace('resource_associations', 'old_resources').replace("'link', 'markdown', 'html'", "'link', 'markdown'"));
  db.exec(`INSERT INTO old_resources SELECT * FROM resource_associations;
    DROP TABLE resource_associations; ALTER TABLE old_resources RENAME TO resource_associations;`);
  db.exec('PRAGMA foreign_keys = ON');
  initializePanelSchema(db);
  initializePanelSchema(db);
  assert.deepEqual(readPanelLayout(db), before);
  assert.equal(db.prepare('PRAGMA foreign_keys').get().foreign_keys, 1);
  assert.deepEqual(db.prepare('PRAGMA foreign_key_check').all(), []);
  const path = join(root, 'preview.HTML');
  writeFileSync(path, '<h1>Preview</h1>');
  let revision = before.revision;
  let html;
  for (const value of ['preview.HTML', path, '~/preview.HTML']) {
    const added = addResource(db, group.id, { kind: 'html', value }, revision, { home: root });
    assert.equal(added.resource.value, path);
    revision = added.revision;
    html = added.resource;
  }
  assert.equal(readPanelLayout(db).groups.find((item) => item.id === group.id).resources.filter((item) => item.kind === 'html').length, 1);
  const opened = openResourcePanel(db, html.id, {}, revision);
  assert.equal(opened.panel.kind, 'iframe');
  assert.equal(opened.panel.markdownMode, null);
  assert.throws(() => updatePanel(db, opened.panel.id, { markdownMode: 'edit' }, opened.revision), /Markdown panel/);
  assert.throws(() => addPanel(db, group.id, { kind: 'markdown', resourceId: html.id }, opened.revision), /markdown resource/);
  for (const [value, status] of [['missing.html', 404], ['preview.txt', 400], ['', 400]]) {
    assert.throws(() => addResource(db, group.id, { kind: 'html', value }, opened.revision), (error) => error.status === status);
  }
  mkdirSync(join(root, 'directory.htm'));
  assert.throws(() => addResource(db, group.id, { kind: 'html', value: 'directory.htm' }, opened.revision), /regular file/);
  assert.equal(removeResource(db, html.id, {}, opened.revision).panel.id, opened.panel.id);
  assert.equal(readFileSync(path, 'utf8'), '<h1>Preview</h1>');
});
