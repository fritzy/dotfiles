...
import { sidebarBranchLabel } from '../web-v2/src/utils.js';
...
test('branch prefix omission is literal, leading, and preserves custom names', () => {
  const shorten = (name, prefixes, branch = name) => sidebarBranchLabel(name, branch, prefixes);
  assert.equal(shorten('fritzy/fix', ['fritzy/']), '…fix');
  assert.equal(shorten('fritzy/team/fix', ['fritzy/', 'fritzy/team/']), '…fix');
  assert.equal(shorten('fritzy/fritzy/fix', ['fritzy/']), '…fritzy/fix');
  assert.equal(shorten('team/fritzy/fix', ['fritzy/']), 'team/fritzy/fix');
  assert.equal(shorten('dev.name/fix', ['dev.name/']), '…fix');
  assert.equal(shorten('devXname/fix', ['dev.name/']), 'devXname/fix');
  assert.equal(shorten('fritzy/fix', []), 'fritzy/fix');
  assert.equal(shorten('fritzy/fix', ['fritzy/fix']), 'fritzy/fix');
  assert.equal(shorten('fritzy/custom label', ['fritzy/'], 'fritzy/branch'), 'fritzy/custom label');
});
...
function mockNetwork({ groupedLocal = false, panelModel = false, prefixedBranches = false, requests = [] } = {}) {
  ...
  globalThis.fetch = async (url) => {
    ...
    if (path.includes('/fw/all/')) {
      const items = panelModel || prefixedBranches ? [
        { id: 1, type: 'repo', repo: 'acme/project', branch: prefixedBranches ? 'fritzy/leaf' : 'leaf', name: prefixedBranches ? 'fritzy/leaf' : 'Leaf', status: 'paused', path: '/tmp/leaf' },
    ...
    if (path.includes('/panel-layout') && panelModel) {
      return {
        ...
        json: async () => ({
          ...
          groups: [
            {
              id: 'leaf-group', type: 'repository', ownerId: '1', label: prefixedBranches ? 'fritzy/leaf' : 'Leaf', path: '/tmp/leaf', panels: [], resources: [],
            },
            {
...
test('session creation controls select and open on their owning machine', async (t) => {
  ...
  assert.ok(
    requests.includes('http://127.1.1.2:7337/fw/new'),
...
for (const panelModel of [true, false]) {
  test(`omitted branch prefixes apply across machines and persist, including empty settings (${panelModel ? 'panel tree' : 'legacy rows'})`, async (t) => {
    let mounted;
    t.after(async () => { await mounted?.unmount(); });
    mounted = await harness(t, { panelModel, prefixedBranches: true });
    const label = (target) => machineSection(mounted.container, target).querySelector(panelModel
      ? '[data-sidebar-panel-group="leaf-group"] > div > button span.truncate'
      : '[data-sidebar-session="1"] span.truncate');
    const assertLabels = (expected) => {
      for (const target of ['local', 'workstation']) {
        assert.equal(label(target).textContent, expected);
        assert.ok(label(target).classList.contains('truncate'), 'overflow uses a trailing ellipsis');
    ...
    async function editPrefixes(value) {
      ...
      const input = mounted.container.querySelector('#sidebar-omitted-branch-prefixes');
      await actCall(() => {
        Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set.call(input, value);
        input.dispatchEvent(new window.Event('input', { bubbles: true }));
      });
      assert.equal(localStorage.getItem('fritzworks-omitted-branch-prefixes'), value);
      await actCall(() => mounted.container.querySelector('[aria-label="Back to sessions"]').click());
    }
    ...
    async function remount() {
      ...
      const React = await import('react');
      const { default: App } = await import('../web-v2/src/App.jsx');
      mounted = await mountReact(React.createElement(App));
      await flush(100);
    }

    await editPrefixes(' team/, fritzy/l ');
    assertLabels('…eaf');
    await remount();
    assertLabels('…eaf');
    await editPrefixes('');
