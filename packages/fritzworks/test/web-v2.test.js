...
test('v2 is an isolated React and Tailwind client using the existing protocol', () => {
  ...
  assert.match(constants, /TERMINAL_MODE_STORAGE_KEY = 'fritzworks-terminal-mode'/);
  assert.match(constants, /SYNC_WINDOW_FULLSCREEN_STORAGE_KEY = 'fritzworks-sync-window-fullscreen'/);
  assert.match(constants, /TERMINAL_FONT_STORAGE_KEY = 'fritzworks-terminal-font'/);
  assert.match(constants, /SIDEBAR_TREE_STORAGE_KEY = 'fritzworks-sidebar-tree'/);
...
test('v2 retains the session controls and creation widgets as React components', () => {
  ...
  assert.match(localTerminal, /\/fw\/terminal/);
