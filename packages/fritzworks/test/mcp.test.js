...
test('MCP exposes daemon discovery and a daemon selector on every tool', async (t) => {
  const dir = mkdtempSync(join(tmpdir(), 'fritzworks-mcp-'));
  ...
  const transport = new StdioClientTransport({
    ...
    env: { FRITZWORKS_CONFIG: configPath },
    stderr: 'pipe',
  });
  const client = new Client({ name: 'fritzworks-test', version: '1.0.0' });
  ...
  assert.ok(tools.some((tool) => tool.name === 'fw_resource_list'));
  assert.ok(tools.some((tool) => tool.name === 'fw_resource_read'));
  assert.ok(tools.some((tool) => tool.name === 'fw_resource_write'));
  assert.ok(tools.some((tool) => tool.name === 'fw_sync'));
  assert.ok(tools.some((tool) => tool.name === 'fw_browser_refresh'));
  assert.equal(tools.some((tool) => tool.name === 'fw_note'), false);
  assert.equal(tools.some((tool) => tool.name === 'fw_note_list'), false);

  const response = await client.callTool({ name: 'fw_daemons', arguments: {} });
  ...
  const browserRefreshResponse = await client.callTool({
    name: 'fw_browser_refresh', arguments: { daemon: 'relay' },
  });
  ...
  const relayedResponse = await client.callTool({
    name: 'fw_list', arguments: { daemon: 'relay', all: true },
  });
  ...
  assert.deepEqual(requests.slice(-1), ['/fw/all?status=all&perpage=100']);

  const syncResponse = await client.callTool({
    name: 'fw_sync', arguments: { daemon: 'relay', workstream: '99' },
  });
  ...
  assert.deepEqual(requestDetails.at(-1), {
    url: '/fw/99/sync', method: 'POST', body: {},
  });

  const resourceResponse = await client.callTool({
    name: 'fw_resource_add',
    arguments: {
  ...
  const htmlResponse = await client.callTool({
    name: 'fw_resource_add',
    arguments: { daemon: 'relay', group: 'session-1', kind: 'html', value: 'preview.html', open: true },
  });
  assert.equal(htmlResponse.isError, undefined);
  assert.deepEqual(requestDetails.at(-1).body, {
    client: 'mcp', revision: 7, kind: 'html', value: 'preview.html', open: true,
  });

  const listedResponse = await client.callTool({
    name: 'fw_resource_list', arguments: { daemon: 'relay', group: 'session-1' },
  });
