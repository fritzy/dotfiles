...
import {
  DEFAULT_OMITTED_BRANCH_PREFIXES, SIDEBAR_TREE_STORAGE_KEY, STANDALONE_TERMINAL_DRAG_TYPE, TERMINAL_FONTS, THEMES,
} from './constants.js';
import { useTarget } from './target-context.js';
import {
  Button, inputClass, selectClass,
} from './ui.jsx';
import {
  branchState, groupActiveSessionsByRepo, sidebarBranchLabel, timestamp,
} from './utils.js';
...
function SessionRow({
  item, selected, highlighted, navigationKey, onHighlight, onActivate, onOpenDetails, rowRef, branchPrefixes,
}) {
  const name = item.name || item.branch || String(item.id);
  const displayName = item.type !== 'scratchpad'
    ? sidebarBranchLabel(name, item.branch, branchPrefixes)
    : name;
...
function PanelGroupNode({
  ...
  rowRef, resourceRowRef, branchPrefixes,
}) {
  const displayName = group.type === 'repository'
    ? sidebarBranchLabel(group.label, session?.branch, branchPrefixes)
    : group.label;
...
export default function ActiveSessionsSidebar({
  ...
  theme, onThemeChange,
  omittedBranchPrefixes = DEFAULT_OMITTED_BRANCH_PREFIXES, onOmittedBranchPrefixesChange,
  ...
  const branchPrefixes = useMemo(() => omittedBranchPrefixes.split(',').map((prefix) => prefix.trim()).filter(Boolean), [omittedBranchPrefixes]);
          ...
          {view === 'sessions' ? (
            ...
            <div id={`${panelName('settings')}-view`} aria-label="Settings" className="grid content-start gap-5 px-2 pt-3 pb-16">
              <section className="grid gap-2">
                <label className="text-sm font-bold text-primary" htmlFor="sidebar-omitted-branch-prefixes">Omitted branch prefixes</label>
                <input
                  id="sidebar-omitted-branch-prefixes"
