...
const MARKDOWN_REHYPE_PLUGINS = [markdownSourceLines];
...
export const MarkdownPreview = memo(function MarkdownPreview({ children }) {
  return (
    <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={MARKDOWN_REHYPE_PLUGINS} components={MARKDOWN_COMPONENTS}>
      {children}
    </ReactMarkdown>
  );
});
...
export default function MarkdownEditor({
  ...
  const [followChanges, setFollowChanges] = useState(true);
  const pendingFollowRef = useRef(null);
  const textareaRef = useRef(null);
  const previewRef = useRef(null);
  const stateRef = useRef({ content: '', saved: '', version: null });
  const dirty = content !== saved;
  stateRef.current = { content, saved, version, preview, followChanges, visible };
  ...
  const load = useCallback(async (signal, { preserveDirty = false } = {}) => {
    ...
    try {
      ...
      if (preserveDirty && current.version !== null && current.preview && current.followChanges && current.visible) {
        const line = firstChangedLine(current.content, file.content);
        if (line !== null) pendingFollowRef.current = { content: file.content, line };
      }
  ...
  useEffect(() => {
    if (!visible || !preview || !followChanges || loading) {
      pendingFollowRef.current = null;
      return undefined;
    }
    const pending = pendingFollowRef.current;
    if (!pending || pending.content !== content) return undefined;
    const timer = setTimeout(() => {
      pendingFollowRef.current = null;
      if (previewRef.current) scrollToMarkdownLine(previewRef.current, pending.line);
    }, FOLLOW_DELAY_MS);
    return () => clearTimeout(timer);
  }, [content, followChanges, loading, preview, visible]);
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      >
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1.5">
          ...
          {preview && (
            <Button
              variant={followChanges ? 'primary' : 'secondary'}
              className="min-h-7 px-2 py-1 text-xs"
              aria-pressed={followChanges}
              title="Scroll to external document changes"
