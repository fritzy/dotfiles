export function sidebarBranchLabel(name, branch, prefixes) {
  if (name !== branch) return name;
  // Prefer the most specific match and remove only one literal leading prefix.
  const prefix = prefixes.filter((value) => value && name.startsWith(value))
    .sort((a, b) => b.length - a.length)[0];
  return prefix && name.length > prefix.length ? `…${name.slice(prefix.length)}` : name;
}
