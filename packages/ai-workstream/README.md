...
The REST `resume` command reconstitutes a missing worktree and tells the React client to activate its browser terminal workspace; the first connected PTY marks it active. `pause` disconnects every browser PTY for that session. Workstream detail responses still include live Zellij `panels` state for the legacy Zellij panel controls, but that state no longer affects Active/Paused status. Every configured location supports pause/resume, panel toggles, provider selection, focus, and path opening, while the API always rejects `close` for it.
...
The React client's `test3` drawer contains an xterm.js terminal backed by a local `zsh` PTY. It uses the separate `/ws/terminal` WebSocket for terminal input, output, and resize messages; `/ws/events` is unchanged. Terminal upgrades are accepted only from loopback clients with a matching origin when an origin is supplied. Each browser connection receives a new PTY rooted at the user's home directory, and its shell is killed when the socket disconnects or the daemon stops.

The bottom drawer also hosts a tabbed markdown editor for the work notes under the tree configured as `[locations.notes]`. The "+ note" button lists every markdown file under `work/`, filterable by path, and offers to scaffold this week's work note only while that file does not exist yet — once it does, it is simply one of the listed files. Journal entries and the per-session notes `ws note` writes under `work/<YYYY>/workstream/` are deliberately excluded. That scaffold action creates the file with one `## <Weekday>, <Month> <Day><ord>, <Year>` heading per weekday and with today's heading guaranteed to be present, matching the layout the notes skill expects; an older compact `<YYYYMMDD>-week.md` name is reused rather than duplicated. Each tab has an Edit/Preview toggle, font-size and fullscreen controls, Tab/Shift-Tab indenting, Enter list continuation (a task bullet continues as `- [ ]`, an empty bullet ends the list), a Today button that jumps to today's heading, and an "+ Entry" button that appends a `- [x] ` bullet at the end of today's section. Ctrl-H/Ctrl-L move between panels, Ctrl-K returns to the workspace, and Ctrl-F and Ctrl-P toggle fullscreen and the sidebar, exactly as they do from a terminal. Edits autosave shortly after typing stops and on blur, plus explicitly with Ctrl/Cmd-S. Saves carry the content hash the tab loaded, so a file edited elsewhere in the meantime is reported as a conflict with reload and overwrite choices rather than silently clobbered.
...
PUT  /notes/tabs                       # {"scope":"global","tabs":[{"path":"…"}],"activePath":"…"}
```
...
Every path is resolved inside the notes root — `..`, absolute paths, symlinks pointing outside it, and non-`.md` files are rejected with 400 — and files are capped at 1 MiB. Which notes are open is stored server-side in `<paths.data>/editor-tabs.json` rather than in local storage, so the tab strip comes back after a reload or in another browser. Tabs are restored closed; the drawer only opens when a tab is chosen.

Terminal and note tabs share one strip that rides with the drawer: the whole strip sits at the bottom of the viewport while the drawer is down and travels up with the panel when it opens. The drawer slides back down as soon as it stops being the focused panel — clicking or focusing a session terminal, moving into the sidebar, Escape, or clicking away all collapse it, and the tab strip stays reachable at the bottom.
