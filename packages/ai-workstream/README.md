...
Work-note paths remain confined to the notes root — `..`, absolute paths, symlinks pointing outside it, and non-`.md` files are rejected by the `/notes/file` routes. The explicit `/markdown/file` routes accept Markdown paths elsewhere on the selected daemon but only open existing regular files. Both kinds are capped at 1 MiB. Open editor tabs are stored server-side in `<paths.data>/editor-tabs.json` rather than in local storage, so the tab strip comes back after a reload or in another browser. Tabs are restored closed; the drawer only opens when a tab is chosen.
